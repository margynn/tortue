...
use crate::domain::tracker::{AnnounceRequest, TrackerAnnouncer, TrackerResponse};
...
impl TrackerAnnouncer for TokioTrackerClient {
    async fn announce(&self, req: &AnnounceRequest) -> Option<TrackerResponse> {
        match &self.endpoint {
            Endpoint::Http { url } => http_announce(&self.http, url, req).await.ok(),
            Endpoint::Udp { host, port } => udp_announce(host, *port, req).await.ok(),
        }
    }
}
...
async fn http_announce(
    ...
    let peers = match (decoded.get_bytes(b"peers"), decoded.get_list(b"peers")) {
        ...
        _ => return Err(Error::InvalidTrackerResponse("missing peers field".to_owned())),
    };
...
fn build_http_url(base_url: &Url, req: &AnnounceRequest) -> String {
    ...
    out.push_str(&percent_encode(req.info_hash.as_ref(), TRACKER_ENCODE_SET).to_string());
    out.push_str("&peer_id=");
    out.push_str(&percent_encode(req.peer_id.as_ref(), TRACKER_ENCODE_SET).to_string());
    out
}

fn parse_http_peers_list<'a>(peer_list: &[Bencode<'a>]) -> Result<Vec<SocketAddr>> {
    let mut peers = Vec::new();
    for peer in peer_list {
        let ip_raw = peer.get_bytes(b"ip")?;
        let ip_str = std::str::from_utf8(ip_raw)
            .map_err(|_| Error::InvalidTrackerResponse("peer ip was not valid utf-8".to_owned()))?;
...
async fn udp_announce(host: &str, port: u16, req: &AnnounceRequest) -> Result<TrackerResponse> {
    ...
    socket.send(&build_announce_request(connection_id, announce_tx, req)).await?;
...
fn parse_connect_response(bytes: &[u8], expected_tx_id: u32) -> Result<u64> {
    ...
    if action != ACTION_CONNECT {
        return Err(Error::InvalidTrackerResponse(format!("unexpected udp connect action: {action}")));
    }
    if tx_id != expected_tx_id {
        return Err(Error::InvalidTrackerResponse("udp connect transaction id mismatch".to_owned()));
    }
...
fn parse_announce_response(bytes: &[u8], expected_tx_id: u32) -> Result<TrackerResponse> {
    ...
    if action != ACTION_ANNOUNCE {
        return Err(Error::InvalidTrackerResponse(format!("unexpected udp announce action: {action}")));
    }
    if tx_id != expected_tx_id {
        return Err(Error::InvalidTrackerResponse("udp announce transaction id mismatch".to_owned()));
    }
