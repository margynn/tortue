...
Le coût visible, c'est un invariant que le `Coordinator` doit se rappeler de respecter :

```rust
// coordinator.rs:126 — cette fonction n'existe que pour réconcilier deux états
...
## Problème 2 — La vue du registre de peers est dupliquée, et la copie utilisée est fausse
...
`PeerRegistry::apply` traite `Bitfield` / `Have` / `HaveAll` / `HaveNone` en un seul endroit, retraits compris : `HaveNone` et un `Bitfield` de remplacement appellent `remove_peer` avant de réenregistrer. Le bug est corrigé structurellement, pas par un appel ajouté à la main.
...
`Coordinator` passe de quatre champs d'état à trois : `peer_registry`, `assignments`, `pieces`.

---

## Problème 3 — La politique d'ordonnancement est une propriété émergente

`schedule_requests` (`coordinator.rs:418-479`) c'est un triple nid de boucles, un budget mutable, et quatre collaborateurs mélangés :
...
`on_message_suggest_piece` (`coordinator.rs:389-416`) réimplémente un mini-scheduler :
...
**`PieceEvent` a trois variantes pour deux comportements.** `BlockReceived` et `PieceInvalid` ont un traitement identique (`coordinator.rs:329-330`), et le `match` de `on_message_piece` est imbriqué sur deux niveaux :
